...
if (isset($_POST['client-login'])) {
    ...
    if ($data->num_rows > 0) {
        ...
        session_start();
        $_SESSION['logged'] = ['id' => $data['id_customer'], 'email' => $data['email'], 'username' => $data['username'], 'password' => $data['password'], 'image' => $data['imageData'], 'alamat' => $data['alamat'], 'tgl_lahir' => $data['tgl_lahir']];
